...
Automatic paste needs one of those recording actions on a release bind, so the shortcut is released before the chord — for example:

```sh
bindr = SUPER, D, exec, noctalia msg plugin magus/dictation:controller all toggle
```
...
The chord is not sent until the shortcut that ended the recording has been released, and that release is observed rather than waited out: automatic paste requires the dictation shortcut to be a release bind (`bindr`), because a release bind is dispatched by the compositor when the shortcut key is released. The bind table is read with `hyprctl -j binds` when the recording starts, so a press-triggered shortcut, a bind table the compositor cannot report, and a missing `hyprctl` all hold the transcript with the reason and the bind to add, while **Copy transcript** and **Paste now** stay ungated. The chord is additionally sent no sooner than 500 ms after the job finished. What no rootless client can observe is a modifier you hold for another reason: Hyprland publishes no held-modifier state — `hyprctl devices` reports the lock keys, not ctrl/alt/shift/super — so the plugin does not claim that such a modifier is part of the gate.
...
- Automatic paste also needs the dictation shortcut to be a release bind (`bindr`), which is the one release signal a rootless client can read: the compositor dispatches a release bind when the shortcut key is released, and the bind table is what says so. A press-triggered shortcut is held with the bind to add, and **Paste now** remains the route. The chord is sent 500 ms after the job finished as well. What neither this plugin nor any other rootless client observes is a modifier you hold for another reason: Hyprland exposes no held-modifier state, so a modifier held that way can still reach the paste.
