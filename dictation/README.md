...
- Which file is used follows Hyprland's own precedence: a `--config` path on the compositor's command line first, then `HYPRLAND_CONFIG` when it is set, otherwise `hyprland.lua` in `$XDG_CONFIG_HOME/hypr` (or `~/.config/hypr`) and then in `$XDG_CONFIG_DIRS`, and `hyprland.conf` in the same order. The command line is read with `ps`; when it cannot be read, whether the compositor was started with `--config` is unknown and the setup writes nothing rather than guessing which file to edit. An explicit path that names a file which does not exist is reported as such, and the row offers no chord, rather than claiming a free chord in a file it cannot write.
...
Job directories and their files are private to your user (directory `0700`, files `0600`). Re-running an existing job id writes `attempt-2`, and so on; earlier attempts are kept. Recordings are kept until you delete the job directory: nothing prunes them, and nothing is deleted when a job ends. **Recent dictations** reads these directories and nothing else, so it lists a job the same way after a restart. Files shipped by this plugin are only ever read, never written.
...
- **Play** and **Retry** address the newest recording by default, which is the one a preserved failure leaves behind. **Recent dictations** reaches older ones without importing their files. Nothing is deleted on its own: the history lists the newest 20 jobs and the rest stay on disk, so retention is still yours to manage.
